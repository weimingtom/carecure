...
uses
  ...
  DBLookupEh,upreview,Unit_DataModule, DBSumLst, DBCtrlsEh, sncCurrency, Mask, GridsEh;

type
  Tfrm_GoodsWholeSale = class(TForm)
    DBGridEh1: TDBGridEh;
    Label1: TLabel;
...
procedure Tfrm_GoodsWholeSale.FormShow(Sender: TObject);
begin
...
procedure Tfrm_GoodsWholeSale.DBGridEh1ColExit(Sender: TObject);
begin
   ...
   if  DMod.tkc.Locate('ypid',DMod.tcr.Fieldvalues['ypid'],[])  then
      begin
       if DMod.tkc.FieldByName('数量').AsInteger<DMod.tcr.FieldByName('数量').AsInteger then
        ...
        begin
        DMod.tcr.edit;
        DMod.tcr.FieldByName('单价').Asfloat:=DMod.typzdk.fieldbyname('批发价').asfloat
        end
...
function Tfrm_GoodsWholeSale.check: boolean;
begin
  result:=true;
 ...
 if DMod.tcr.RecordCount<=0 then
   begin
    showmessage('请录入销售的商品信息！');
    result:=false;
    exit;
   end;
end;

procedure Tfrm_GoodsWholeSale.saveClick(Sender: TObject);
begin
 
  if not check then
  exit;
  DMod.ctemp.Parameters.ParamValues['@lx']:='pf';
  DMod.ctemp.Execute;
  ...
  begin
    ...
    DMod.tcr.FieldByName('类型').Asstring:='批发';
   ...
   begin
   fpreview.frrk.LoadFromFile(frm_UserLogin.apppath+'\print\pf.frf');
...
procedure Tfrm_GoodsWholeSale.retClick(Sender: TObject);
begin
...
procedure Tfrm_GoodsWholeSale.FormClose(Sender: TObject; var Action: TCloseAction);
begin
try
...
procedure Tfrm_GoodsWholeSale.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_GoodsWholeSale.sfExit(Sender: TObject);
var aa:real;
begin
...
procedure Tfrm_GoodsWholeSale.DBGridEh1ColEnter(Sender: TObject);
begin
sf.Value:=0;
zq.Value:=0;
end;

procedure Tfrm_GoodsWholeSale.dwChange(Sender: TObject);
begin
if trim(dw.Text)<>'' then
 begin
...
procedure Tfrm_GoodsWholeSale.dwExit(Sender: TObject);
begin
...
procedure Tfrm_GoodsWholeSale.renChange(Sender: TObject);
begin
if trim(ren.Text)<>'' then
 begin
...
procedure Tfrm_GoodsWholeSale.BitBtn3Click(Sender: TObject);
begin
close;
end;
procedure Tfrm_GoodsWholeSale.renExit(Sender: TObject);
begin
