...
procedure Tfrm_StoreCheck.FormShow(Sender: TObject);
begin
  DMod.tcr.Close;
  DMod.tcr.Open;
...
procedure Tfrm_StoreCheck.DBGridEh1ColExit(Sender: TObject);
begin
...
function Tfrm_StoreCheck.check: boolean;
begin
  result:=true;
...
procedure Tfrm_StoreCheck.saveClick(Sender: TObject);
begin
 
  if not check then
  exit;
  DMod.ctemp.Parameters.ParamValues['@lx']:='pd';
  DMod.ctemp.Execute;
  ...
  begin
    ...
    if DMod.tcr.FieldByName('数量').AsInteger<0 then
    begin
    DMod.tcr.FieldByName('入库单位').AsInteger:=-2;
    DMod.tcr.FieldByName('类型').Asstring:='盘亏';
    DMod.tcr.fieldbyname('出入').asboolean:=false;
    end
     else
     begin
    DMod.tcr.FieldByName('出库单位').AsInteger:=-2;
    DMod.tcr.FieldByName('类型').Asstring:='盘盈';
...
procedure Tfrm_StoreCheck.retClick(Sender: TObject);
begin
...
procedure Tfrm_StoreCheck.FormClose(Sender: TObject; var Action: TCloseAction);
begin
action:=cafree;
end;

procedure Tfrm_StoreCheck.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_StoreCheck.BitBtn3Click(Sender: TObject);
begin
close;
end;
