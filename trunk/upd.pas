...
begin
  ...
  begin
    ...
    if adodm.tcr.FieldByName('数量').AsInteger<0 then
    begin
    adodm.tcr.FieldByName('入库单位').AsInteger:=-2;
    adodm.tcr.FieldByName('类型').Asstring:='盘亏';
    adodm.tcr.fieldbyname('出入').asboolean:=false;
    end
     else
     begin
    adodm.tcr.FieldByName('出库单位').AsInteger:=-2;
    adodm.tcr.FieldByName('类型').Asstring:='盘盈';
